...
bot.onText(/\/zt/, (msg) => {
  ...
  connection.query('UPDATE groups SET now = ? WHERE chatid = ?', [timeoutMap[chatId][1], chatId], (error, results) => {
    ...
    if (error) {
      ...
      appendErrorToFile('chatId为'+chatId+'停止推送视频消息时发生错误:'+error.message);
...
function sendVideoMessages(chatId, now) {
  ...
  connection.query('SELECT * FROM videos', (error, results) => {
    if (error) {
     ...
     appendErrorToFile('查询视频消息时发生错误:'+error.message);
      return;
    }
...
appendErrorToFile('ChatId为'+chatId+'更新 now 字段时发生错误:'+error.message);
        }
      });
...
appendErrorToFile('ChatId为'+chatId+'视频消息发送失败:'+error.message);
        });
